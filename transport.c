...
#include <signal.h>
#include <syslog.h>
#define DEBUG( format...) _debug(__FILE__, __LINE__,LOG_ERR, format)
void _debug(const char *filename, int line, int level, const char *format, ...)
{
    va_list vlist;
    sigset_t block_chld;

    openlog("adb", LOG_PID, LOG_DAEMON);
...
retry:
    DEBUG("serial %s  type %d  err %s\n",serial,ttype,error_out == NULL?"false":*error_out);
    if (error_out)
...
ttype = kTransportAny;
 DEBUG("type %d",ttype);
    ...
    if (result) {
        DEBUG("____6__%d_",result->connection_state);
    ...
    DEBUG("serial2 %s  err %s\n",serial,error_out == NULL?"false":*error_out);
